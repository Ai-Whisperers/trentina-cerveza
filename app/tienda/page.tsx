...
export const metadata: Metadata = {
  title: c.tienda.seo.title,
  description: c.tienda.seo.description,
};

const site = c.site;

function WhatsAppButton({ href, text }: { href: string; text: string }) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-2 px-5 py-2.5 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-all text-sm"
    >
      <svg
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        {pack.originalPrice && (
                          <span className="text-xs text-[var(--color-text-muted)] line-through mr-2">
                            {pack.originalPrice}
                          </span>
                        )}
                      </div>
        ...
        <div className="container-page">
          <div className="max-w-lg mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-[var(--font-heading)] font-bold text-[var(--color-text)] mb-4">
              ¿Tenés dudas sobre los packs?
            </h2>
            <p className="text-[var(--color-text-muted)] mb-8">
              Consultanos al WhatsApp y te asesoramos sobre el pack ideal para vos
            </p>
            <a
